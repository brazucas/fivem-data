...
function GetNameAndID(source)
	local user_id = vRP.getUserId(source)
	return GetPlayerName(source) .. " (" .. vRP.getFriendlyIdByUserId(user_id) .. ")" or '?'
end
...
AddEventHandler('_chat:messageEntered', function(author, color, message, channel)
	...
	local user_id = vRP.getUserId(source)
    TriggerEvent('chatMessage', source, author .. " (" .. vRP.getFriendlyIdByUserId(user_id) .. ")", message, channel)

    if not WasEventCanceled() then
        TriggerClientEvent('chatMessage', -1, author .. " (" .. vRP.getFriendlyIdByUserId(user_id) .. ")",  { 255, 255, 255 }, message)
    end
...
AddEventHandler('__cfx_internal:commandFallback', function(command)
    local name = GetNameAndID(source)
...
AddEventHandler('chat:init', function()
	local name = GetNameAndID(source)
    TriggerClientEvent('chatMessage', -1, '', { 255, 255, 255 }, '^2* ' .. name .. '^r^2 entrou.')
end)

AddEventHandler('playerDropped', function(reason)
	local name = GetNameAndID(source)
    TriggerClientEvent('chatMessage', -1, '', { 255, 255, 255 }, '^2* ' .. name .. '^r^2 saiu (' .. reason .. ')')
end)
...
AddEventHandler('poodlechat:whisperMessage', function(id, message)
	...
	if id then
		...
		TriggerClientEvent('poodlechat:whisperEcho', source, id, GetNameAndID(id), message)
...
function StaffMessage(source, message)
	if not IsPlayerAceAllowed(source, ServerConfig.StaffChannelAce) then
		TriggerClientEvent('chat:addMessage', source, {
			color = {255, 0, 0},
			args = {'Erro', 'Você não tem permissão para acessar o canal da staff.'}
		})
		return
	end
...
function SendReportToDiscord(source, id, reason)
	...
	local message = table.concat({
		...
		'**Licença:** ' .. reporterLicense,
		'**IP:** ' .. reporterIp,
		'',
		'**Jogador reportado:** ' .. reporteeName,
		'**Licença:** ' .. reporteeLicense,
		'**IP:** ' .. reporteeIp,
		'',
		'**Motivo:** ' .. reason
	}, '\n')
		...
		PerformHttpRequest(
			...
			function(status, text, headers)
				...
				if IsResponseOk(status) then
					...
					Log('error', string.format('Falha ao enviar reporte: %d %s %s\n%s', status, text, json.encode(headers), message))
...
AddEventHandler('poodlechat:report', function(player, reason)
	if not IsDiscordReportEnabled() then
		TriggerClientEvent('chat:addMessage', source, {
			color = {255, 0, 0},
			args = {'Erro', 'A função de reportar não está habilitada.'}
		})
		return
	end
	...
	if id then
		...
		TriggerClientEvent('chat:addMessage', source, {
			color = {255, 0, 0},
			args = {'Erro', 'Não existe o ID ou jogador com o nome ' .. player}
		})
	end
end)

AddEventHandler('playerJoining', function()
	SendToDiscord("**" .. GetName(source) .. "** está conectando-se ao servidor.", 65280)
end)

AddEventHandler('playerDropped', function(reason) 
	local color = 16711680
	if string.match(reason, "Chutado") or string.match(reason, "Banido") then
		color = 16007897
	end

	SendToDiscord("**" .. GetName(source) .. "** desconectou-se do servidor. \n Motivo: " .. reason, color)
end)
	...
	PerformHttpRequest(
		...
		function(status, text, headers)
			if IsResponseOk(status) then
				...
				Log('warning', string.format('Falha ao receber mensagens: %d %s %s', status, text, json.encode(headers)))
			end
	...
	PerformHttpRequest(
		...
		function(status, text, headers)
			if IsResponseOk(status) then
				...
				Log('success', 'Pronto para receber mensagens do Discord!')
			else
				Log('error', string.format('Falha ao inicializar: %d %s %s', status, text, json.encode(headers)))
			end
