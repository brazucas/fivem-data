...
RegisterNetEvent("PV:vehicleID")
AddEventHandler("PV:vehicleID", function(vehicle)
	vehicleSpawning = NetToVeh(vehicle)
end)
...
_Utils.CreateVehicle = function(model, pos, props)
	...
	vSERVER.spawnVehiclesOutside(GetDisplayNameFromVehicleModel(model), props.plate, pos)
	--TriggerServerEvent("brz:spawnVehiclesOutside",GetDisplayNameFromVehicleModel(model), props.plate, pos)

	while vehicleSpawning == nil do
		Wait(1)
	end
	local vehicle = vehicleSpawning
	vehicleSpawning = nil
