...
AddEventHandler("character-creator:finishedCharacter", function(characterNome, characterSobrenome, characterAge, currentCharacterMode)
    ...
    if user_id then
        ...
        local p = promise.new()
        exports.mongodb:updateOne({ collection = "vrp_user_identities", query = { user_id = user_id }, update = { ["$set"] = { firstname = "Indigente", name = "Individuo", age = characterAge } }}, function(success, result)
            p:resolve(success)
        end)
        Citizen.await(p)
