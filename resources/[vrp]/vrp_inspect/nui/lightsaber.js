$(document).ready(function(){
	...
	document.onkeyup = function(data){
		if (data.which == 27){
			$.post("https://vrp_inspect/chestClose");
		}
	};
});
...
const updateDrag = () => {
	...
	$('.esquerda').droppable({
		...
		drop: function(event,ui){
			...
			$.post("https://vrp_inspect/takeItem", JSON.stringify({
				item: itemData.key,
	...
	$('.direita').droppable({
		...
		drop: function(event,ui){
			...
			$.post("https://vrp_inspect/storeItem", JSON.stringify({
				item: itemData.key,
				amount: Number($("#amount").val())
			}))
...
const updateChest = () => {
	$.post("https://vrp_inspect/requestChest",JSON.stringify({}),(data) => {
