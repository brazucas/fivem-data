        ...
        if args[1] == "add" and homes[tostring(args[2])] then
            ...
            if myHomes[1] then
                ...
                exports.mongodb:insertOne({
                    collection = "vrp_homes_permissions",
                    document = {
                        user_id = parseInt(args[3]),
                ...
                if userHomes[1] then
                    if vRP.tryFullPayment(user_id, 50000) then
                        exports.mongodb:updateOne({
                            collection = "vrp_homes_permissions",
                            query = { user_id = arseInt(args[3]), home = tostring(args[2]) },
                            update = { ["$set"] = { garage = true } }
                        })
                        local identity = vRP.getUserIdentity(parseInt(args[3]))
                        if identity then
        ...
        elseif args[1] == "transfer" and homes[tostring(args[2])] then
            ...
            if myHomes[1] then
                ...
                if identity then
                    ...
                    if ok then
                        exports.mongodb:delete({ collection = "vrp_homes_permissions", query = { home = tostring(args[2]) } })
                        exports.mongodb:insertOne({
                            collection = "vrp_homes_permissions",
                            document = {
                                home = tostring(args[2]),
        ...
        elseif args[1] == "tax" and homes[tostring(args[2])] then
            ...
            if ownerHomes[1] then
                ...
                if vRP.tryFullPayment(user_id, parseInt(house_price * house_tax)) then
                    exports.mongodb:deleteOne({ collection = "vrp_homes_permissions", query = { user_id = parseInt(ownerHomes[1].user_id), home = tostring(args[2]) } })
                    exports.mongodb:insertOne({
                        collection = "vrp_homes_permissions",
                        document = {
                            home = tostring(args[2]),
                            user_id = parseInt(ownerHomes[1].user_id),
                            owner = true,
                            tax = parseInt(os.time()),
                            garage = true,
                        }
                    })
