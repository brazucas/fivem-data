...
function vRP.getUserAddress2(user_id)
    local p = promise.new()
    exports.mongodb:findOne({ collection = "vrp_user_homes", query = { user_id = user_id } }, function(success, results)
        if success then
            if (#results > 0) then
                p:resolve(results[1])
            else
...
function vRP.setUserAddress(user_id, home, number)
    local p = promise.new()
    local home = vRP.getUserAddress(user_id)

    if string.len(uData) > 0 then
        exports.mongodb:updateOne({ collection = "vrp_user_homes", query = { _id = home._id }, update = { ["$set"] = { home = home, number = number } } }, function(success, result)
            p:resolve(success)
        end)
    else
        exports.mongodb:insertOne({ collection = "vrp_user_homes", document = { user_id = user_id, home = home, number = number } }, function(success, result, insertedIds)
            if success then
                p:resolve(insertedIds[1])
            else
                p:reject("[MongoDB][vRP.setUserAddress] Error in insertOne: " .. tostring(result))
            end
        end)
    end
...
function vRP.getUserByAddress(home, number)
    local p = promise.new()
    exports.mongodb:findOne({ collection = "vrp_user_homes", query = { home = home, number, number } }, function(success, results)
        if success then
            if (#results > 0) then
                p:resolve(results[1])
            else
    ...
    local home = Citizen.Await(p)

    if #home > 0 then
        return home[1].user_id
    end
